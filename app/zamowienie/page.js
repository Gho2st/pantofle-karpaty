...
export default function CheckoutPage() {
  ...
  const { cartItems, fetchCart, clearCart } = useCart(); // Zakładam, że dodasz clearCart do kontekstu
    ...
    try {
      ...
      const p24Response = await fetch("/api/p24/create", {
        ...
        body: JSON.stringify({
          ...
          amount: calculateTotal() * 100, // W groszach
      ...
      if (!p24Response.ok) throw new Error(p24Data.error);

      // Przekieruj do P24 z tokenem
      const p24Url = p24Data.redirectUrl;
      window.location.href = p24Url; // Pełne przekierowanie

      // Opcjonalnie: wyczyść koszyk po sukcesie (ale lepiej po weryfikacji)
      // clearCart();
    } catch (error) {
