...
export default function CollectionTitle({
  ...
  const displayPrice = promoPrice !== undefined ? promoPrice : price;
  ...
  return (
    ...
    >
      <div className={imageContainerClasses}>
        ...
        {/* PASEK Z % RABATU – PÓŁPRZEZROCZYSTY, CZERWONY */}
        {hasPromo && (
          <div
            className="absolute top-3 left-3 bg-red-600/90 text-white text-xs font-bold px-3 py-1.5 rounded-full shadow-lg backdrop-blur-sm z-10 
                       transform -rotate-12 transition-all duration-300 group-hover:rotate-0 group-hover:scale-110"
          >
            -{discountPercent}%
          </div>
        )}
      ...
      {/* CENA */}
      {(price !== undefined || promoPrice !== undefined) && (
        <div className="text-center pb-4 space-y-1">
          {/* Przekreślona cena */}
          {hasPromo && price !== undefined && (
          ...
          {lowestPrice !== undefined && lowestPrice < displayPrice && (
